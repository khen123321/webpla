...
class Profile(models.Model):
    # CHANGED: on_delete=models.CASCADE ensures Profile is deleted when User is deleted.
...
class ClaimRequest(models.Model):
    ...
    # CHANGED: on_delete=models.CASCADE ensures claim requests are deleted when User is deleted.
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='claim_requests')
    
    # ClaimRequests are strongly tied to Rewards. If a Reward is deleted, claims for it should also delete.
    ...
    # We use SET_NULL here: If the admin who *processed* the claim is deleted, the claim history remains, but the processed_by field becomes NULL.
